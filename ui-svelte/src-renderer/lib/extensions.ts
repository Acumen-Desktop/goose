...
export async function addExtension(
  ...
  try {
    ...
    const errorMessage = `Failed to add ${extension.name} extension: ${
...
export async function removeExtension(
  ...
  try {
    ...
    const errorMessage = `Error removing ${name} extension${
      data.message ? `. ${data.message}` : ""
    }`;
    console.error(errorMessage);
    toast.error(errorMessage);
    ...
    const errorMessage = `Failed to remove ${name} extension: ${
      error instanceof Error ? error.message : "Unknown error"
    }`;
    console.error(errorMessage);
    toast.error(errorMessage);
...
export async function loadAndAddStoredExtensions() {
  try {
    ...
    console.error(
      "Error loading and activating extensions from localStorage: ",
      error
    );
  }
}
...
function envVarsRequired(config: ExtensionConfig) {
  return (config.env_keys?.length ?? 0) > 0;
}
...
export async function addExtensionFromDeepLink(url: string) {
  ...
  const allowedCommands = ["npx", "uvx", "goosed"] as const;
  if (!allowedCommands.includes(cmd as (typeof allowedCommands)[number])) {
    handleError(
      `Failed to install extension: Invalid command: ${cmd}. Only ${allowedCommands.join(
        ", "
      )} are allowed.`,
      true
    );
  ...
  if (!id || !name || !description) {
    handleError(
      "Failed to install extension: Missing required parameters",
      true
    );
    return;
  }
  ...
  const config: FullExtensionConfig = {
    ...
    type: "stdio",
    cmd,
    args: parsedUrl.searchParams.getAll("arg"),
    description,
    enabled: true,
  ...
  if (envVarsRequired(config)) {
    ...
    window.location.href = `/settings?extensionId=${config.id}&showEnvVars=true`;
    return;
  }
